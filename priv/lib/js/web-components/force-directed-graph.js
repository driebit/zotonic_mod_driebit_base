...
class ForceDirectedGraph extends HTMLElement {
    ...
    connectedCallback() {
        this.getData(this.resourceId).then(response => {
            ...
            const isDevEnv = /docker|test/i.test(window.location.href);

            if (isDevEnv) {
                console.error('ForceDirectedGraph.js load error:', error);
            } else {
                console.log("Something went wrong while loading the force directed graph.");
            }
        });
    }
